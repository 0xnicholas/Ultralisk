...
使用 PyO3 进行绑定，Rust 代码编译为 Python 可调用的 `.so` 动态库。vLLM 已有 C++ 扩展的先例（`vllm/_C.abi3.so`），Zealot 的 Rust 组件用同样的机制注入：

```
Python 代码：
  import zealot_rs  # PyO3 编译的 Rust 库

  block_manager = zealot_rs.BlockManager(num_gpu_blocks=1024, block_size=16)
  block_id = block_manager.allocate()  # Rust 内部管理引用计数
...
Rust 代码：
  ...
  #[pymethods]
  impl BlockManager {
      fn allocate(&self) -> PyResult<usize> { ... }
  }
```
...
- Rust 组件独立演进，Python 侧可以分阶段退役

**负面：**
- Python ↔ Rust FFI 引入序列化开销（PyO3 零拷贝可缓解）
- 团队需要同时掌握 Python + Rust + CUDA 三套工具链
- Rust Block Manager 和 Python Scheduler 之间的交互边界需要精心设计
