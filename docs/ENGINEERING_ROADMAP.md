...
> **依据**: ADR-000 ~ ADR-010  
> **周期**: 18 个月，4 个 Phase  
> **同步规则**: 本文是里程碑的权威来源。ADR 变更影响排期时需同步更新本文。产品路线图 `docs/roadmap.md` 的 Phase 归属和时间点以本文为准。
...
─────                    ──────                    ──────                  ──────
• Gateway (Rust)         • vLLM → Zealot 切换    • UIE 1.0 stable         • 客户私有模型
...
**目标**: 从"开发者工具"到"企业平台"。Zealot 独立引擎启动，A/B 灰度替代 vLLM。Reserved 策略上线。
...
Gateway ────────────┬─────────────────┬──────────────────┬
  多实例部署          │ 攒批迁至 Redis  │ Route Table CRD  │
                    │                 │                  │
Engine ─────────────┼─────────────────┼──────────────────┼
  招聘 GPU 工程师    │ Attention kernel│ Prefill-Decode    │ ✅ v2 原型
  Zealot 引擎启动    │ 优化 v1         │ 分离原型验证      │
                    │                 │                  │
Control Plane ──────┼─────────────────┼──────────────────┼
...
### M4 — Operations 模块 + Zealot 引擎启动

| 工作流 | 交付内容 | ADR |
|--------|---------|-----|
| **Gateway** | 多实例部署。Batch 攒批迁至 Redis 共享队列。路由表 weight 字段启用（A/B 测试预备） | 002, 010 |
| **Engine** | Zealot 引擎启动。Block Manager + Constrained Decode Rust 实现。vLLM 基线保留，A/B 灰度准备。2-3 名 GPU/CUDA 工程师已到位 | 003, 009, 010 |
...
|------|------|
| GPU 利用率 | > 50% |
| Zealot 吞吐提升 | > 20% vs vLLM vanilla |
| 企业客户占比（>5 人团队） | > 30% |
