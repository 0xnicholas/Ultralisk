...
Runtime Interface 支持任意引擎可替换接入。当前部署策略：

```
Phase 1:     Gateway → vLLM Backend
Phase 2:     Gateway → vLLM Backend (基线) + Zealot Backend (A/B 灰度)
Phase 3:     Gateway → Zealot Backend (默认)
```
...
| 引擎 | 角色 | 状态 |
|------|------|------|
| **vLLM** | 通用推理基线，社区最成熟 | Phase 1 部署中 |
| **Zealot** | 自研引擎，Block Manager + CUDA kernel 优化 | Phase 2 构建 |

> 备选：SGLang 在结构化生成（JSON mode / tool calling）场景有 2-3x 加速，可作为特定场景补充，不在当前路线内。
...
### 4.5 Zealot 语言栈：Rust 做主机，Python 做加载，CUDA 做计算

Zealot 是 Rust 主进程，Python 作为嵌入解释器仅用于 HuggingFace Model Loader。
...
```
              ...
              │         Zealot 内部语言分层          │
              │                                  │
  Rust (主) ─►│  Block Manager (KV cache 管理)   │ ← Phase 2
              │  Scheduler                       │ ← Phase 2（vLLM 等价逻辑）
              │  Constrained Decode Engine       │ ← Phase 2
              │  gRPC Server (tonic)             │ ← Phase 2
              │                                  │
  Python ────►│  Model Loader (HF 生态)          │ ← 永久保留，PyO3 嵌入
              │                                  │
...
**为什么不直接干掉 Python**：HuggingFace 生态全体在 Python，重写 Model Loader 等于放弃整个模型生态。但 Python 只在模型启动时执行一次（加载权重），decode loop 全程 Rust native。
...
### 4.6 Backend Runtime：引擎可替换抽象

Gateway 和 Control Plane 不耦合到任何具体引擎。所有推理能力通过 **Runtime Interface** (gRPC) 暴露，每个引擎实例化为一个 Backend Runtime。
...
Gateway / Control Plane
  ...
  └──┬─────┬───┘
     │     │
  ┌──▼──┐ ┌▼──────┐
  │vLLM │ │Zealot │  ← 两个 Backend
  │HTTP │ │native │     vLLM: Rust sidecar gRPC→HTTP
  └──┬──┘ └┬──────┘     Zealot: native trait，零转换
     │      │
  vLLM    Zealot Pod

备选 Backend（proto 已预留，未部署）：SGLang
```
...
| 推理引擎 | vLLM 基线 → Zealot 自研 | Phase 1 用开源快速上线，Phase 2 发布 Zealot 独立引擎，Phase 3 发布 Zealot 1.0 | 2026-07-11 |
