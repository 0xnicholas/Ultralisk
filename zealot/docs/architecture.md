...
Zealot 是 Ultralisk 的自研推理引擎，目标是 Phase 3 替代 vLLM，性能达到 Together TIE 的 80%+。

**策略**：从 vLLM fork 起步，**组件级替换**关键路径——Python 做兼容，Rust 做内核，CUDA 做计算。不是全量重写。

```
Phase 2（注入）                     Phase 3（独立）
vLLM fork ──────────────────────────────────────────────── Zealot Backend
  │                                                          │
  ├─ import zealot_engine (.so)                               ├─ tonic gRPC Server
  ├─ Python Scheduler                                         ├─ Rust Scheduler
  ├─ Python Block Manager  ← replaced                         ├─ Rust Block Manager
  └─ Python Constrained Decode ← replaced                     └─ Rust Constrained Decode
```
...
### 4.1 Phase 2 流程（当前实现目标）

```
1. API Server (Python)
   - 接收 HTTP 请求，tokenize prompt → seq of token IDs
   - 调用 Python Scheduler.allocate_seq()

2. Python Scheduler
   - 决定调度策略（vLLM 原版 continuous batching）
   - 调用 BlockManager.allocate_blocks(seq_len)  ← Rust FFI

3. Block Manager (Rust)
   - 从 free pool 分配 GPU block，返回 block_table
   - 返回 BlockHandle (generation-gated typed handle，见 §6.1)

4. Prefill (CUDA) — vLLM 原版
   - 并行处理全部 prompt tokens，构建 KV cache，产生第一个 output token

5. Decode Loop（反复执行）:
   a. Attention kernel (CUDA) — Phase 2 改装
      - 通过 block_table 索引读取 KV cache，计算 logits

   b. Constrained Decode (Rust) — Phase 2
      - 如果请求含 json_schema：DFA 校验 token，裁剪 logits

   c. Sampling (CUDA) — vLLM 原版

   d. Python Scheduler
      - 决定继续 decode 或加入下一个 batch

6. Stream Complete
   - Python Scheduler.free_seq()
   - BlockManager.drop(BlockHandle) → refcount -1，归零回收
```

### 4.2 Phase 3 流程（目标状态）

与 Phase 2 的区别：Scheduler 从 Python 迁移到 Rust 内核，Python 仅保留 API Server 和 Model Loader。decode loop 内不再穿越 Python/Rust 边界（见 §7 演进路径）。
...
通过 PyO3，Rust 代码编译为 `zealot_engine.abi3.so`，Python 直接 import。

**Phase 2 注入（临时）**：
```python
try:
    from zealot_engine import BlockManager
except ImportError:
    from vllm.core.block_manager_v1 import BlockSpaceManagerV1 as BlockManager
```
...
| 结构 | Python 侧 | Rust 侧 | 传递方式 |
|------|----------|---------|---------|
| `block_table` | `List[int]` | `Vec<usize>` | PyO3 自动转换（零拷贝） |
| `token_ids` | `List[int]` | `Vec<i32>` | PyO3 自动转换 |
| `schedule_result` | `Dict` | `struct ExecutionBatch` | PyO3 #[pyclass] 导出 |
| `BlockHandle` | 不透明对象 | typed handle | 按值传递，含 generation counter |

### 5.3 GIL 策略

此节是设计决策的核心权衡点。Block Manager 在 decode loop 中被高频调用（每 step 每 seq），需要量化 GIL 持有时间及其对并发的影响。

**Phase 2 的 GIL 模型**：

| 组件 | GIL 状态 | 理由 | 风险 |
|------|---------|------|------|
| Block Manager | 持有 GIL | Phase 2 只有 Python Scheduler 调用，单线程执行，无并发 | Phase 3 多线程后成为瓶颈 |
| Constrained Decode | 持有 GIL | 每 step 被 Python sampler 同步调用 | 同上 |
| Scheduler (Python) | 持有 GIL | vLLM 原生 Python 调度器 | 已存在于 vLLM，非 Zealot 引入 |

**Phase 3 的 GIL 模型**：

| 组件 | GIL 状态 | 实现方式 |
|------|---------|---------|
| Block Manager | 释放 GIL | `Python::allow_threads(|| bm.allocate())` |
| Constrained Decode | 释放 GIL | 同上 |
| Scheduler | 释放 GIL | 同上，原生 Rust 无 GIL |

**GIL 持有时间预算**（M4 验收指标）：

| 操作 | 目标持有时间 | 测量方式 |
|------|------------|---------|
| `allocate()` | < 500ns (含 free list 查找) | `cargo bench` + `criterion` |
| `free()` | < 200ns | 同上 |
| `reference()` | < 100ns | 同上 |
| `allowed_tokens()` | < 5μs (取决于 schema 复杂度) | 同上 |

> 如果 M4 实测中 Block Manager 单次操作超过 1μs，则 Phase 2 即启用 `allow_threads`，不等 Phase 3。
...
Phase 2（M4-M6）：注入模式
  - vLLM fork 仍存在
  - Block Manager (Rust .so) 替换 vLLM Python 实现
  - Constrained Decode CPU 侧 (Rust .so) 替换 vLLM outlines 库
  - Attention / Quant kernel 改装 (CUDA)
  - Python Scheduler 仍保留
  - GIL：Block Manager 持有 GIL（单线程 Scheduler，无竞争）

Phase 3（M7-M12）：独立模式
  - vLLM fork 不再存在
  - Zealot Backend = 独立进程（tonic gRPC Server）
  - Scheduler 从 Python → Rust（allow_threads 释放 GIL）
  - API Server + Model Loader 脱离 fork，纳入 Zealot 仓库
  - Block Manager Free List 从 Mutex<Vec> 升级为无锁方案
  - 性能达标（Together TIE 80%+）
```

| 里程碑 | 标志 | 交付 |
|--------|------|------|
| M4 | Block Manager 通过 PyO3 注入 vLLM fork | Generation-gated handle，运行时错误检测 |
| M5 | Constrained Decode + Attention kernel v1 | 缓存方案 + DoS 防护，20-30% 吞吐提升 |
| M6 | Prefill-Decode 分离原型 | GPU 利用率 > 50% |
| M7-M8 | Rust Scheduler + Full Constrained Decode | allow_threads GIL 释放，GC tail latency 消除 |
| M9-M10 | UIE 1.0 alpha | 内部 benchmark 2x vLLM vanilla |
| M11-M12 | UIE 1.0 stable | 默认引擎，脱离 vLLM fork |
...
- vLLM Python Scheduler 已有 swap-out 机制：将 KV cache 从 GPU 迁移到 CPU，释放 GPU block。Zealot Block Manager 只需提供 `free()` 接口，Scheduler 决定 swap 哪些 seq。
- 如果 swap 后仍无法满足，`OutOfBlocks` 上抛到 Python 层，触发 vLLM 内置的抢占逻辑（preempt by recomputation）。

### Phase 3 策略（设计预览，M7 细化）
