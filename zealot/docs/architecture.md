...
```
┌─────────────────────────────────────────────────┐
│         Zealot 内部三层（单一 Rust 进程）          │
│                                                  │
│  Layer 1: Rust 入口层                             │
│  ┌────────────────────────────────────────────┐  │
│  │ tonic gRPC Server（Runtime Interface，ADR-010）│ │
│  └───────────────────┬────────────────────────┘  │
│  Layer 2: Rust Core（内核层）                    │
│  ┌────────────────────────────────────────────┐  │
│  │ ┌──────────┐ ┌──────────┐ ┌─────────────┐  │  │
│  │ │ Scheduler │ │  Block   │ │ Constrained │  │  │
│  │ │(prefill/ │ │ Manager  │ │   Decode    │  │  │
│  │ │ decode)  │ │          │ │             │  │  │
│  │ └─────┬─────┘ └────┬─────┘ └──────┬──────┘  │  │
│  │ ┌─────┴─────┐ ┌────┴─────┐              │    │  │
│  │ │ Sampling  │ │Tokenizer │              │    │  │
...
│  └────────────────────────────────────────────┘  │
│                                                  │
│  嵌入式 Python（PyO3，进程内，仅启动时使用）        │
│  ┌────────────────────────────────────────────┐  │
│  │ Model Loader（HuggingFace 生态）            │  │
│  └────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────┘
```

**分界原则**：
- Layer 1（Rust）：tonic gRPC Server 接收 Runtime Interface 请求。OpenAI 兼容在 Gateway 完成，不在 Zealot 内。
- Layer 2（Rust）：decode loop 全程 Rust native，通过 FFI 调用 CUDA kernel，不触及 Python。
- Layer 3（CUDA）：vLLM 原版 kernel 的改装优化，不改架构只调参。
- 嵌入式 Python：PyO3 在启动时调用 HuggingFace Model Loader 加载权重到 GPU，完成后释放 GIL 休眠（详见 §5）。
...
Client Request
    ...
    ▼
┌──────────────────┐
│  gRPC Server     │  Layer 1 (Rust, tonic) — 接收 InferRequest
│  (tonic)         │  （Runtime Interface proto，含 token IDs）
└────────┬─────────┘
         │ token IDs, params
         ▼
┌──────────────────┐
│  Scheduler       │  Layer 2 (Rust) — 消费 ExecutionPlan，决定调度
│                  │  Phase 2 逻辑等价 vLLM continuous batching
└────────┬─────────┘
...
|--------|------|------|------------------------|
| Rust 内部（Scheduler → Block Manager） | 分配/释放 block | 直接函数调用 | < 200ns |
...
> **dev-mode 例外（GPU 到位前）**：当前 `PyModelRunner`（`src/model_runner_py.rs`）用 torch CPU 前向跑真实推理，Python 在 decode loop 内、每 step 穿越 FFI。这是无 GPU 环境下的临时形态，用于验证 Scheduler/Engine/Runtime Interface 全链路；GPU kernel 就位后 runner 换为 Rust+CUDA 实现，Python 回归"仅启动时加载"的本节设计。
...
| 里程碑 | 标志 | 交付 |
|--------|------|------|
| M4 | Zealot Backend 独立进程 + Block Manager | Generation-gated handle，Runtime Interface gRPC；Scheduler（continuous batching 等价 + OOM 抢占）与 dev-mode CPU runner 已提前落地 |
...
| 层级 | 框架 | 覆盖 | 运行环境 |
|------|------|------|---------|
| Rust 单元测试 | `cargo test --lib` | Block Manager, Scheduler, Engine, Sampling, Tokenizer, Constrained Decode | CI（无 GPU） |
...
zealot/                     # 源码仓库
  ...
  ▼
target/release/
  zealot-backend            # 独立 Rust 主进程（tonic gRPC + PyO3 嵌入 CPython）

# M4 当前形态：Rust 组件编译为 cdylib Python 扩展，GPU-free 开发与测试
...
**依赖链**：`zealot-backend` → CUDA (libcuda.so) + CPython（PyO3 嵌入，仅 Model Loader）→ 无其他 Rust 运行时依赖（静态链接）。
...
| 风险 | 缓解 |
|------|------|
| 新模型适配慢于 vLLM 社区（1-2 周） | Model Loader 走 HF transformers，升级 Python 依赖即获得支持；CI 跟踪 transformers 新版本 |
