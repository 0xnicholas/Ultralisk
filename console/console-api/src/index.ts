...
app.get('/v1/admin/endpoints/:id', (req, res) => { const ep = MOCK_ENDPOINTS.find((e) => e.id === req.params.id); if (!ep) return res.status(404).json({ error: { code: 'not_found', message: 'Endpoint not found' } }); res.json({ data: ep }); });
...
app.patch('/v1/admin/endpoints/:id', (req, res) => { const idx = MOCK_ENDPOINTS.findIndex((e) => e.id === req.params.id); if (idx === -1) return res.status(404).json({ error: { code: 'not_found', message: 'Endpoint not found' } }); MOCK_ENDPOINTS[idx] = { ...MOCK_ENDPOINTS[idx], ...req.body }; res.json({ data: MOCK_ENDPOINTS[idx] }); });
app.delete('/v1/admin/endpoints/:id', (req, res) => { const idx = MOCK_ENDPOINTS.findIndex((e) => e.id === req.params.id); if (idx === -1) return res.status(404).send(); MOCK_ENDPOINTS.splice(idx, 1); res.status(204).send(); });
...
app.get('/v1/admin/batch-jobs/:id', (req, res) => { const job = MOCK_BATCH_JOBS.find((j) => j.id === req.params.id); if (!job) return res.status(404).json({ error: { code: 'not_found', message: 'Batch job not found' } }); res.json({ data: job }); });
...
app.delete('/v1/admin/batch-jobs/:id', (req, res) => { const idx = MOCK_BATCH_JOBS.findIndex((j) => j.id === req.params.id); if (idx === -1) return res.status(404).send(); MOCK_BATCH_JOBS.splice(idx, 1); res.status(204).send(); });
...
app.patch('/v1/admin/sessions/:id', (req, res) => { const idx = MOCK_SESSIONS.findIndex((s) => s.id === req.params.id); if (idx === -1) return res.status(404).json({ error: { code: 'not_found', message: 'Session not found' } }); MOCK_SESSIONS[idx] = { ...MOCK_SESSIONS[idx], ...req.body, updated_at: new Date().toISOString() }; res.json({ data: MOCK_SESSIONS[idx] }); });
app.delete('/v1/admin/sessions/:id', (req, res) => { const idx = MOCK_SESSIONS.findIndex((s) => s.id === req.params.id); if (idx === -1) return res.status(404).send(); MOCK_SESSIONS.splice(idx, 1); res.status(204).send(); });
