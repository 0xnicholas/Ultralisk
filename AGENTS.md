...
- **Zealot**: P2 components done — Block Manager (generation-gated handles) + Constrained Decode (JSON schema → DFA) + Scheduler (priority queue, block budget, OOM preemption, prefill-decode separation). `zealot-backend` is a real Runtime Interface server running an Engine actor with Rust Sampler (temperature, top-k/p, repetition/frequency/presence penalties) + lightweight Rust Tokenizer (byte-level BPE decoder). `PyModelRunner` returns raw logits; Engine's Sampler handles the sampling in Rust. 49/49 unit + 2/2 backend e2e + 1/1 real-model CPU e2e pass. CUDA kernels, Batch optimization, RadixAttention, Speculative Decoding still open.
